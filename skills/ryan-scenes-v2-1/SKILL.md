...
- resolve any missing narrative-focus, photographic-scope, and intervention-level preferences in one compact plain-text menu before generation;
...
- Treat a preference as resolved when the user has already stated it clearly. Ask only the unresolved questions, but place every unresolved question in the same plain-text block. Never ask them sequentially.
...
### Required Plain-Text Menu

Always conduct this gate in ordinary conversation text. Do not call `open_scene_preferences`, `request_user_input`, another MCP UI, or any interactive choice/form tool even when one is available. This text-only rule is the defining interaction behavior of v2.1.

Present every unresolved question once in one compact numbered block. Never ask the questions sequentially. Use three lettered choices per question, put the recommended choice first, mark it `（推荐）`, and keep every label and description short. Ask the user to reply in a compact form such as `1A 2B 3A`; adapt the numbering when fewer than three questions remain. Accept natural-language and custom answers as well. Wait for the user's reply before composing the final prompt or generating; recommendations are not consent and must never be selected silently.
...
3. Run the Pre-Generation Preference Gate once as a compact plain-text menu for every unresolved preference and wait for the user's reply.
...
```markdown
![Ryan scenes v2.1 poster](absolute-image-path-or-rendered-image)
...
- Were all unresolved preferences requested once before generation, with every unresolved question grouped in one compact plain-text block?
...
- Were plugin UI, `request_user_input`, and all other interactive choice tools avoided even if available, and did generation wait for the user's textual reply?
